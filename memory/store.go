...
func NewSQLiteStore(config StoreConfig) (*SQLiteStore, error) {
	...
	if err := os.Chmod(config.DBPath, 0600); err != nil {
		db.Close()
		return nil, fmt.Errorf("failed to protect memory database: %w", err)
	}
...
func (s *SQLiteStore) Add(embedding *VectorEmbedding) error {
	...
	// Resolve optional table availability before beginning the transaction.
	// With a single SQLite connection, querying meta after Begin would wait
	// for the connection already held by this transaction.
...
func (s *SQLiteStore) insertFTS(tx *sql.Tx, embedding *VectorEmbedding) error {
	if _, err := tx.Exec(`DELETE FROM memory_fts WHERE id = ?`, embedding.ID); err != nil {
		return err
	}
	_, err := tx.Exec(
		`INSERT INTO memory_fts (text, id, source, type) VALUES (?, ?, ?, ?)`,
		embedding.Text,
		embedding.ID,
		embedding.Source,
		embedding.Type,
...
func (s *SQLiteStore) Search(query []float32, opts SearchOptions) ([]*SearchResult, error) {
	...
	// Portable fallback for the pure-Go SQLite build. The local hash embedder
	// keeps this deterministic and requires no loadable SQLite extension.
	return s.searchBruteForce(query, opts)
}

func (s *SQLiteStore) searchBruteForce(query []float32, opts SearchOptions) ([]*SearchResult, error) {
	rows, err := s.db.Query(`
		SELECT id, text, source, type, embedding, created_at, updated_at,
	...
	for rows.Next() {
		...
		if err != nil || len(embedding.Vector) != len(query) {
			continue
		}
		if !memorySourceAllowed(embedding.Source, opts.Sources) ||
			!memoryTypeAllowed(embedding.Type, opts.Types) {
			continue
		}
		score, err := CosineSimilarity(query, embedding.Vector)
		if err != nil {
			continue
		}
		// Signed feature hashing can produce a small negative similarity.
		if score < 0 {
			score = 0
		}
...
func (s *SQLiteStore) Get(id string) (*VectorEmbedding, error) {
	...
	ve, err := scanMemoryEmbedding(s.db.QueryRow(`
		SELECT
...
func (s *SQLiteStore) List(filter func(*VectorEmbedding) bool) ([]*VectorEmbedding, error) {
	...
	for rows.Next() {
		ve, err := scanMemoryEmbedding(rows)
		if err != nil {
			continue
		}
...
func scanMemoryEmbedding(row memoryScanner) (*VectorEmbedding, error) {
	var embedding VectorEmbedding
	var embeddingJSON, tagsJSON, filePath, sessionKey sql.NullString
	var lastAccessed, lineNumber, accessCount sql.NullInt64
	var importance sql.NullFloat64
	var createdAt, updatedAt int64
	if err := row.Scan(
		&embedding.ID,
	...
	embedding.CreatedAt = time.Unix(createdAt, 0)
	embedding.UpdatedAt = time.Unix(updatedAt, 0)
	if embeddingJSON.Valid {
		_ = json.Unmarshal([]byte(embeddingJSON.String), &embedding.Vector)
		embedding.Dimension = len(embedding.Vector)
	}
	embedding.Metadata.FilePath = filePath.String
	embedding.Metadata.LineNumber = int(lineNumber.Int64)
	embedding.Metadata.SessionKey = sessionKey.String
	embedding.Metadata.Importance = importance.Float64
	embedding.Metadata.AccessCount = int(accessCount.Int64)
	if tagsJSON.Valid {
		_ = json.Unmarshal([]byte(tagsJSON.String), &embedding.Metadata.Tags)
	}
	if lastAccessed.Valid {
		embedding.Metadata.LastAccessed = time.Unix(lastAccessed.Int64, 0)
	}
	return &embedding, nil
}

func memorySourceAllowed(source MemorySource, allowed []MemorySource) bool {
	if len(allowed) == 0 {
		return true
	}
...
func memoryTypeAllowed(memoryType MemoryType, allowed []MemoryType) bool {
	if len(allowed) == 0 {
		return true
	}
	for _, value := range allowed {
