...
test("artifact hashes prove databases and logs are preserved", async () => {
  ...
  assert.deepEqual(Object.keys(before), [
    "canonical-web.log",
    "multica-canonical.db",
    "multica-canonical.db-journal",
    "multica-canonical.db-shm",
    "multica-canonical.db-wal",
  ]);
...
test("listener ownership accepts descendants and rejects legacy or orphan processes", () => {
  const backend = {
    ...
    args: [
      ...
      "./cmd/server",
      "-sqlite-path",
      "C:/repo/data/multica-canonical.db",
    ],
    cwd: "C:/repo/backend",
  ...
  const web = {
    ...
    command: "node",
    args: [
      "C:/repo/apps/web/node_modules/next/dist/bin/next",
      "dev",
      "--port",
      "3000",
    ],
    cwd: "C:/repo",
    creationTime: "web-start",
  };
  ...
  const processes = [
    {
      ...
      creationTime: "backend-start",
      commandLine:
        "go run ./cmd/server -sqlite-path C:/repo/data/multica-canonical.db",
    },
    { pid: 11, parentPid: 10, commandLine: "canonical-server.exe" },
    {
      ...
      creationTime: "web-start",
      commandLine:
        "node C:/repo/apps/web/node_modules/next/dist/bin/next dev --port 3000",
    },
    { pid: 21, parentPid: 20, commandLine: "node next-server" },
  ];
  assert.doesNotThrow(() =>
    validateListenerOwnership(manifest, processes, [
      { port: 8000, pid: 11 },
      { port: 9000, pid: 11 },
      { port: 3000, pid: 21 },
    ])
      ...
      validateListenerOwnership(
        ...
        [
          ...processes,
          {
            pid: 30,
            parentPid: 1,
            commandLine: "go run C:/repo/server/cmd/sqlite-server",
          },
        ],
        []
      ),
      ...
      validateListenerOwnership(
        ...
        [
          ...processes,
          {
            pid: 40,
            parentPid: 1,
            commandLine: "go run ./cmd/server -sqlite-path C:/other/data.db",
          },
        ],
        [
          { port: 8000, pid: 11 },
      ...
      validateListenerOwnership(
        ...
        [
          ...processes,
          {
            ...
            parentPid: 1,
            commandLine:
              "go run D:/other/backend/cmd/server -sqlite-path D:/other/data.db",
          },
        ],
        [
          { port: 8000, pid: 11 },
  ...
  assert.throws(
    () =>
      validateListenerOwnership(
        manifest,
        processes.map((item) =>
          item.pid === 10 ? { ...item, creationTime: "reused-pid" } : item
        ),
        [
          { port: 8000, pid: 11 },
          { port: 9000, pid: 11 },
  ...
  assert.throws(
    () =>
      validateListenerOwnership(
        {
          ...manifest,
          processes: [{ ...backend, cwd: "D:/other/backend" }, web],
        },
        processes,
        [
          { port: 8000, pid: 11 },
        ...
        processes.map((item) =>
          item.pid === 10
            ? {
                ...item,
                commandLine:
                  "go run ./cmd/server -sqlite-path D:/other/data.db",
              }
            : item
        ),
        [
...
test("process evidence rejects legacy or unexpected owners", () => {
  ...
  assert.throws(
    () =>
      validateProcessEvidence(
        { ...valid, database: "C:\\repo\\data\\other.db" },
        "C:\\repo"
      ),
    /database/
  );
  assert.throws(
    () => validateProcessEvidence({ ...valid, mode: "legacy" }, "C:\\repo"),
    /not canonical/
  );
  assert.throws(
    () =>
      validateProcessEvidence(
        {
          ...valid,
          processes: [
            { name: "legacy-backend", cwd: "C:\\repo\\server", args: [] },
          ],
        },
        "C:\\repo"
      ),
  ...
  const foreignBackend = {
    ...backend,
    args: backend.args.map((value) => value.replace("C:\\repo", "D:\\foreign")),
    cwd: "D:\\foreign\\backend",
  };
  ...
  assert.throws(
    () =>
      validateProcessEvidence(
        {
          ...valid,
          database: "D:\\foreign\\data\\multica-canonical.db",
          processes: [foreignBackend, web],
        },
        "C:\\repo"
      ),
    /database|repository/
  ...
  const foreignWeb = {
    ...web,
    args: web.args.map((value) => value.replace("C:\\repo", "D:\\foreign")),
    cwd: "D:\\foreign",
  };
  ...
  assert.throws(
    () =>
      validateProcessEvidence(
        { ...valid, processes: [backend, foreignWeb] },
        "C:\\repo"
      ),
    /repository/
  );
...
test("Canonical config enforces the complete frozen capability matrix", () => {
  ...
  for (const [key, expected] of Object.entries(feature_flags)) {
    assert.throws(
      () =>
        validateCanonicalConfig({
          feature_flags: { ...feature_flags, [key]: !expected },
        }),
      new RegExp(key)
    );
  }
...
test("rollback hash phases require a quiescent runtime with no fixed listeners", () => {
  ...
  assert.throws(
    () => validateQuiescentEvidence({ mode: "canonical" }, []),
    /process manifest/
  );
  assert.throws(
    () => validateQuiescentEvidence(null, [{ port: 9000, pid: 44 }]),
    /listener.*9000/
  );
  assert.throws(
    () =>
      validateQuiescentEvidence(
        null,
        [],
        [{ pid: 55, commandLine: "go run D:/other/backend/cmd/server" }]
      ),
    /orphan.*cmd\/server/
  );
});
