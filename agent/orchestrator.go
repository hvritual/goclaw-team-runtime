...
func (o *Orchestrator) streamAssistantResponse(ctx context.Context, state *AgentState) (AgentMessage, error) {
	...
	if o.config.ContextBuilder != nil {
		...
		if index, projectID, query := latestCatalogQuery(state.Messages); query != "" {
			catalogContext, memoryIDs, err := o.config.ContextBuilder.BuildCatalogContext(projectID, query)
			if err != nil {
				logger.Warn("Failed to retrieve approved catalog memory", zap.Error(err))
			} else if strings.TrimSpace(catalogContext) != "" {
				systemPrompt += "\n\n---\n\n" + catalogContext
				if index >= 0 {
					if state.Messages[index].Metadata == nil {
						state.Messages[index].Metadata = make(map[string]any)
					}
					state.Messages[index].Metadata["catalog_memory_ids"] = memoryIDs
				}
			}
		}
...
func latestCatalogQuery(messages []AgentMessage) (int, string, string) {
	for index := len(messages) - 1; index >= 0; index-- {
		if messages[index].Role != RoleUser {
			continue
		}
		query := extractTextContent(messages[index])
		projectID := ""
		if messages[index].Metadata != nil {
			projectID, _ = messages[index].Metadata["project_id"].(string)
		}
		return index, projectID, strings.TrimSpace(query)
	}
	return -1, "", ""
}
...
func (o *Orchestrator) executeToolCalls(ctx context.Context, toolCalls []ToolCallContent, state *AgentState) ([]AgentMessage, []AgentMessage) {
	...
	for _, tc := range toolCalls {
		...
		if err != nil {
			...
			resultMsg.Content = []ContentBlock{TextContent{Text: err.Error()}}
		}
