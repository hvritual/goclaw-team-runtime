...
// HarnessSource keeps the agent package independent from the concrete
// Better-Harness registry implementation.
type HarnessSource interface {
	ActiveInstructions() (version string, projectID string, content string, componentFiles []string, err error)
}

// CatalogSource exposes only approved, bounded prompt context. The concrete
// catalog implementation remains outside the agent package.
type CatalogSource interface {
	BuildApprovedContext(projectID, query string, limit int) (content string, recordIDs []string, err error)
}
...
// SetHarnessSource installs a versioned, runtime-loadable harness.
func (b *ContextBuilder) SetHarnessSource(source HarnessSource) {
	b.harness = source
}

func (b *ContextBuilder) SetCatalogSource(source CatalogSource) {
	b.catalog = source
}

func (b *ContextBuilder) BuildCatalogContext(projectID, query string) (string, []string, error) {
	if b.catalog == nil {
		return "", nil, nil
	}
	return b.catalog.BuildApprovedContext(projectID, query, 0)
}

// HarnessIdentity returns the currently active harness metadata.
func (b *ContextBuilder) HarnessIdentity() (version string, projectID string, componentFiles []string) {
	if b.harness == nil {
		return "", "", nil
	}
	version, projectID, _, componentFiles, err := b.harness.ActiveInstructions()
	if err != nil {
		return "", "", nil
	}
	return version, projectID, componentFiles
}
...
func (b *ContextBuilder) buildSystemPromptWithSkills(skillsContent string, mode PromptMode) string {
	...
	// 8.1 Versioned Better-Harness components. These are deliberately loaded
	// after workspace facts so policy remains explicit and independently
	// revertible without rewriting the large built-in system prompt.
	if b.harness != nil {
		version, projectID, content, _, err := b.harness.ActiveInstructions()
		if err != nil {
			logger.Warn("Failed to load active harness", zap.Error(err))
		} else if strings.TrimSpace(content) != "" {
			parts = append(parts, fmt.Sprintf(
				"## Active Better Harness\n\nVersion: `%s`\nProject: `%s`\n\n%s",
				version,
				projectID,
				content,
			))
