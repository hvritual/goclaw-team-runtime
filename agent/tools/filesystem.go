...
func (t *FileSystemTool) isAllowed(path string) bool {
	...
	// Resolve symlinks before boundary checks so an allowed directory cannot
	// expose a denied sibling through a link.
	for _, denied := range t.deniedPaths {
		resolvedDenied, err := canonicalPath(denied)
		if err == nil && pathWithin(resolvedDenied, resolvedPath) {
			return false
		}
	}
...
func canonicalPath(path string) (string, error) {
	...
	// For a not-yet-created file, resolve the closest existing ancestor and
	// append only the unresolved lexical suffix.
	current := absolute
	var suffix []string
	for {
