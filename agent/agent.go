...
// RefreshSystemPrompt reloads the active harness before a new run. Existing
// sessions retain their trace-level harness version, while new runs immediately
// pick up a promoted or rolled-back version.
func (a *Agent) RefreshSystemPrompt() {
	a.SetSystemPrompt(a.context.BuildSystemPrompt(nil))
}
