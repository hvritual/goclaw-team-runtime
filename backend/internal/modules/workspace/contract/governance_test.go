...
func TestMutationResultValidate(t *testing.T) {
	valid := MutationResult{ResourceRevision: 1, ResponseStatus: 201, ResponseBody: json.RawMessage(`{"id":"task-1"}`)}
...
func TestAuditRecordValidate(t *testing.T) {
	...
	valid := AuditRecord{ID: "audit-1", Identity: identity, Action: "workspace.task.create", ResourceKind: "task", ResourceID: "task-1", ResourceRevision: 1, OccurredAt: time.Unix(1, 0).UTC(), Metadata: json.RawMessage(`{"status":"todo"}`)}
	if err := valid.Validate(); err != nil {
...
func TestOutboxEventValidate(t *testing.T) {
	...
	valid := OutboxEvent{State: OutboxReady, AvailableAt: now, WorkspaceID: "workspace-1", ID: "event-1", EventType: "task:created", AggregateKind: "task", AggregateID: "task-1", AggregateRevision: 1, Payload: json.RawMessage(`{"id":"task-1"}`), ActorType: "member", ActorID: "member-1", CreatedAt: now}
	if err := valid.Validate(); err != nil {
