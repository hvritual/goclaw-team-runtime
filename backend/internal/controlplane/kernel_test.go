...
func TestKernelPersistsChainAndCommandIdempotency(t *testing.T) {
	...
	if first.Head != 1 || first.Replayed {
	...
	if projection.Head != 1 || projection.Nodes[node.ID].State != node.State {
	...
	if stableProjectionDigest(again) != firstDigest {
...
func TestKernelTamperingAndUnknownEventsFailClosed(t *testing.T) {
	...
	if _, err := kernel.UpsertNode(context.Background(), actor, "command-1", "project-1", 0,
		WorkNode{ID: "task-1", Kind: "task", Revision: 1, State: "draft", CreatorID: actor.ID}); err != nil {
		t.Fatal(err)
	}
	sqlStore := repository.(*sqlRepository)
	if _, err := sqlStore.db.Exec(`UPDATE cp_session_events SET payload = '{"tampered":true}' WHERE workspace_id = 'workspace-1'`); err != nil {
		t.Fatal(err)
	}
...
func TestWorkGraphRejectsDependencyCycle(t *testing.T) {
	...
	for index, id := range []string{"task-1", "task-2"} {
		if _, err := kernel.UpsertNode(ctx, actor, "node-"+id, "project-1", int64(index), WorkNode{ID: id, Kind: "task", Revision: 1, State: "draft", CreatorID: actor.ID}); err != nil {
			t.Fatal(err)
		}
	}
	if _, err := kernel.AddEdge(ctx, actor, "edge-1", "project-1", 2, WorkEdge{ID: "edge-1", From: "task-1", To: "task-2", Kind: "depends_on"}); err != nil {
		t.Fatal(err)
	}
...
func TestDoneGateUsesChecksEvidenceAndIndependentHuman(t *testing.T) {
	...
	if _, err := kernel.UpsertNode(ctx, creator, "command-1", "project-1", 0, node); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := kernel.RecordCheck(ctx, checker, "command-4", "project-1", 2, check); err != nil {
		t.Fatal(err)
	}
...
func openTestKernel(t *testing.T, path string) (*DeliveryKernel, Repository) {
	...
	kernel, err := NewDeliveryKernel(store, func() time.Time { fixed = fixed.Add(time.Second); return fixed }, func(_ context.Context, actor Actor, permission string) error {
		if actor.Kind == ActorAgent && permission == PermissionAccept {
			return ErrDenied
		}
		return nil
	})
