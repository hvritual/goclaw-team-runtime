...
func TestSQLiteRuntimeAdvertisesHonestIssueCapabilities(t *testing.T) {
	...
	if response.Code != http.StatusOK || !containsJSON(response.Body.Bytes(), `"issue_list":true`, `"issue_base_detail":true`, `"issue_detail_pull_requests":false`, `"issue_timeline":true`, `"issue_members":true`, `"issue_reactions":true`, `"issue_subscribers":true`, `"issue_metadata":true`, `"issue_realtime":true`) {
