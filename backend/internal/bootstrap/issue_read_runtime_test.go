...
func TestSQLiteRuntimeAdvertisesHonestIssueCapabilities(t *testing.T) {
	...
	if response.Code != http.StatusOK || !containsJSON(response.Body.Bytes(), `"issue_list":true`, `"issue_base_detail":true`, `"issue_detail_pull_requests":false`, `"issue_timeline":false`, `"issue_members":false`, `"issue_metadata":false`, `"issue_realtime":false`) {
