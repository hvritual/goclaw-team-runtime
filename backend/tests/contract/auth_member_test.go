...
func TestAuthMemberAdaptersShareContract(t *testing.T) {
	...
	request := contract.ProvisionWorkspaceOwnerRequest{}
	localResult, localErr := extension.Local().ProvisionWorkspaceOwner(context.Background(), request)
	...
	grpcResult, grpcErr := auth.NewMemberGRPCClient(connection).ProvisionWorkspaceOwner(context.Background(), request)
