# Product capability roadmap v44 — Release 4 S08A governed selective integration
...
- Predecessor: immutable plan_v43.md / r048 is validation-blocked-before-product-code because its plan-freeze commit has one trailing blank line
...
- Status: draft — R49.1 independent review pending; R49.2-R49.7 unauthorized
- Authority: The Human Customer explicitly confirmed selective integration into codex/multica-six-domain-baseline on 2026-08-27. This successor repairs only the r048 plan-freeze whitespace-validation block; it grants no broader product, deployment, or source-branch authority.
...
R49 depends on the Release 3 closure at the exact base; the three named source commits resolving locally; a clean isolated worktree on the named integration branch; the current repository-owned Node, Go, pnpm, SQLite, and Windows-race toolchains; and a fresh independent R49.1 SPEC PASS. It does not depend on any external service, deployment credential, external database, or legacy tree.

At R49.1, R49.2 start, R49.5 candidate freeze, and R49.7 acceptance, revalidate the exact base, plan v44 SHA-256, policy-bundle hashes, named source commits, and the source path-set SHA-256 recorded in the mutable task register. Stop if any identity differs.
...
This is not a branch merge and not adoption of the source branch's roadmap history. R49 may transplant only the 34 product/test paths below from the three named commits, plus plan_v44.md, plan.md, story-map.md, task-register.md, and journal.md. All source backend/docs history and every source path outside this list are excluded.

The source-manifest identity is the SHA-256 of the following 34 exact path lines
in their listed order, encoded as UTF-8 and terminated with one LF. Source
selection is compared as an unordered set first, then reserialized in this
frozen listed order before calculating the recorded manifest hash.
...
The Customer approved a narrow TDD transplant exception: only the selected, already-authored source tests may travel with their selected implementation; they are not rewritten as new RED tests. The exception ends at this manifest. R49.3-R49.5 must still directly verify all resulting behavior and regressions.

Every R49 commit must contain the continuous Git trailers Task-ID: PCR-001-S08A-R049, Project-ID: PRODUCT-CAPABILITY-ROADMAP, Task-Revision: r049, Work-Item: PCR-S08A, Plan-ID: PRODUCT-CAPABILITY-ROADMAP-001, Plan-Version: 44, a specific Plan-Step, Issue: PCR-S08A, and the frozen Policy-Bundle value above. No unrelated fix may be mixed into the candidate.

## Ordered execution and deterministic acceptance

### R49.1 — Freeze and independently review this successor plan

Record v44/policy/source-manifest identities in the task register and obtain a fresh independent SPEC PASS. Acceptance: all identities resolve, git diff --check passes for roadmap files, the scope is exact, and no product byte is changed before PASS.

### R49.2 — Selectively transplant S08A implementation and tests
...
### R49.3 — Direct feature verification
...
### R49.4 — Canonical regression and static verification
...
### R49.5 — Freeze exact candidate and revalidate identity

Capture candidate/base/tree identity, revalidate all R49 dependency hashes, and verify the worktree is clean apart from the manifest. Acceptance: exact candidate scope and deterministic gate evidence are indexed in the journal.

### R49.6 — Independent dual review
...
### R49.7 — Fast-forward reviewed candidate into canonical
