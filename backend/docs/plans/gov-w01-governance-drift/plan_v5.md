# Governance and Architecture Drift Repair Plan v5
...
- `AGENTS.md`
- `pnpm-lock.yaml` (remove one stale `packages/views` importer only)
- `CLAUDE.md`
- `CONTEXT-MAP.md`
...
Unified CI Run `31545671877` proved that the canonical branch already has a
frozen-lockfile mismatch: `packages/views/package.json` removed
`react-qr-code@catalog:`, while its lockfile importer still declares it.
Version v4 adds only `pnpm-lock.yaml` to S03 for the mechanical removal of that
single stale importer entry. It does not allow dependency upgrades, catalog
changes, package manifest changes, `--no-frozen-lockfile`, or product code.

## Amendment from v4

CI Run `31545817909` showed that canonical baseline does not contain the legacy
`server/**` runtime or `apps/mobile`, while the inherited main-branch jobs still
assume both. Version v5 narrows the stable canonical required gate to:
`governance-policy`, `canonical-backend`, and path-aware `frontend`. Legacy
server, Windows legacy runtime, and installer regressions remain main-branch
reporting jobs and cannot veto the canonical integration branch. Frontend Turbo
filters now exclude mobile only when `apps/mobile` exists. This is a CI contract
correction only; it grants no product-code or `server/**` scope.
