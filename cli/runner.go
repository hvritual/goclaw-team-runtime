...
func newRunnerRegisterCommand() *cobra.Command {
	var id, name, keyFile, verificationSandbox string
	var projects, capabilities []string
	var reuseKey bool
	command := &cobra.Command{
		Use:   "register",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			var err error
			metadata := workstation.RunnerRegistrationMetadata()
			...
			capabilities = workstation.RunnerRegistrationCapabilities(capabilities)
			var key []byte
			if reuseKey {
...
func newRunnerWorkCommand() *cobra.Command {
	var id, keyFile, workRoot, projectID, codexCommand, codexModel string
...
func newRunnerDoctorCommand() *cobra.Command {
	var keyFile, workRoot, codexCommand string
