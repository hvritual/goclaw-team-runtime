...
func runBuiltinIndex(workspace string, cfg *config.Config) {
	...
	// The builtin index uses a deterministic offline embedder. This avoids
	// requiring a second API credential when the LLM itself uses Codex OAuth.
	provider := memory.NewLocalHashProvider()
