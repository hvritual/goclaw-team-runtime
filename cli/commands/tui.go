...
func NewTUIAgent(
	...
	if catalogService != nil {
		_ = toolRegistry.RegisterExisting(tools.NewCatalogMemorySearchTool(catalogService))
		_ = toolRegistry.RegisterExisting(tools.NewCatalogMemoryProposalTool(catalogService))
	}
...
func runTUI(cmd *cobra.Command, args []string) {
	...
	if cfg.Memory.Catalog.Enabled {
		catalogService, err = catalog.NewService(cfg.Memory.Catalog)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Failed to create memory catalog: %v\n", err)
			os.Exit(1)
		}
		defer catalogService.Close()
		catalogService.SetGovernancePolicy(cfg.Governance)
