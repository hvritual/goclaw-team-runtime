// Package sqlite adapts the project-requirements application ports to SQLite.
package sqlite
...
// ApprovalHook persists the approval side effect in the same SQLite transaction.
type ApprovalHook func(context.Context, *sql.Tx, projectrequirements.Record) error

// Repository implements the requirement-baseline application port with SQLite.
type Repository struct {
	db         *sql.DB
	onApproved ApprovalHook
}

// New constructs a SQLite requirement-baseline repository without approval side effects.
...
// Get loads one baseline record scoped to its workspace and project.
...
// CreateDraft atomically writes the first baseline revision.
func (r *Repository) CreateDraft(ctx context.Context, input projectrequirements.SaveDraftInput) (projectrequirements.Record, error) {
...
// ApplyDraft persists a draft plan after retaining compare-and-swap safety.
func (r *Repository) ApplyDraft(ctx context.Context, input projectrequirements.SaveDraftInput, plan domain.DraftPlan) (projectrequirements.Record, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return projectrequirements.Record{}, err
	}
	...
	if plan.CreatesRevision && record.Baseline.Status != projectrequirements.StatusApproved {
		return projectrequirements.Record{}, projectrequirements.ErrRevisionConflict
	}
	if !plan.CreatesRevision && record.Baseline.Status != projectrequirements.StatusDraft {
		return projectrequirements.Record{}, projectrequirements.ErrRevisionConflict
	}
	...
	if plan.CreatesRevision {
		if _, err := tx.ExecContext(ctx, `UPDATE project_requirement_baseline
			...
			plan.Status, plan.NextRevision, now, record.Baseline.ID, input.ExpectedRevision); err != nil {
			return projectrequirements.Record{}, err
		}
		...
		) VALUES (?, ?, ?, ?, ?, ?)`, record.Baseline.ID, plan.NextRevision, content, input.ChangeSummary, input.ActorID, now); err != nil {
			return projectrequirements.Record{}, err
		}
	} else {
		result, err := tx.ExecContext(ctx, `UPDATE project_requirement_revision
			SET content = ?, change_summary = ?, actor_id = ?, created_at = ?
			WHERE baseline_id = ? AND revision = ?`, content, input.ChangeSummary, input.ActorID, now, record.Baseline.ID, plan.NextRevision)
		if err != nil {
			return projectrequirements.Record{}, err
		}
...
// ApplyTransition persists a domain-approved lifecycle transition atomically.
func (r *Repository) ApplyTransition(ctx context.Context, input projectrequirements.TransitionInput, transition domain.Transition) (projectrequirements.Record, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return projectrequirements.Record{}, err
	}
	...
	if transition.Revision != input.ExpectedRevision || record.Baseline.CurrentRevision != transition.Revision {
		return projectrequirements.Record{}, projectrequirements.ErrRevisionConflict
	}
	if record.Baseline.Status != transition.From {
		return projectrequirements.Record{}, projectrequirements.ErrInvalidTransition
	}
	now := timestamp()
	approved := transition.ApprovesRevision()
	result, err := tx.ExecContext(ctx, `UPDATE project_requirement_baseline
		...
		transition.To, approved, transition.ApprovedRevision,
		transition.RecordsSubmission, input.ActorID, transition.ClearsSubmission,
		transition.RecordsSubmission, now, transition.ClearsSubmission,
		approved, input.ActorID,
		approved, now,
		now, record.Baseline.ID, transition.From, transition.Revision)
	if err != nil {
		return projectrequirements.Record{}, err
	}
	...
	switch {
	case approved:
		_, err = tx.ExecContext(ctx, `UPDATE project_requirement_revision SET approved_by = ?, approved_at = ? WHERE baseline_id = ? AND revision = ?`, input.ActorID, now, record.Baseline.ID, transition.Revision)
	case transition.RecordsSubmission:
		_, err = tx.ExecContext(ctx, `UPDATE project_requirement_revision SET submitted_by = ?, submitted_at = ? WHERE baseline_id = ? AND revision = ?`, input.ActorID, now, record.Baseline.ID, transition.Revision)
	case transition.ClearsSubmission:
		_, err = tx.ExecContext(ctx, `UPDATE project_requirement_revision SET submitted_by = NULL, submitted_at = NULL WHERE baseline_id = ? AND revision = ?`, record.Baseline.ID, transition.Revision)
	}
	if err != nil {
		return projectrequirements.Record{}, err
	}
	if approved && r.onApproved != nil {
...
//nolint:funlen,gocognit // The explicit SQL-to-domain mapping keeps persistence representations at this boundary.
func (*Repository) get(ctx context.Context, q rowQuerier, workspaceID, projectID string) (projectrequirements.Record, error) {
