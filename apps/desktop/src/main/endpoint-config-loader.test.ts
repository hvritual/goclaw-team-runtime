...
describe("loadEndpointConfig", () => {
  ...
  it("parses a valid packaged desktop.json", async () => {
    ...
    await expect(
      loadEndpointConfig({ isDev: false, configPath, env: {} }),
    ).resolves.toEqual({
      ok: true,
      config: {
        schemaVersion: 1,
  ...
  it("fails closed when packaged desktop.json is invalid", async () => {
    ...
    const result = await loadEndpointConfig({ isDev: false, configPath, env: {} });

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.message).toContain(configPath);
      expect(result.error.message).toContain("Invalid desktop endpoint config JSON");
    }
  });
});
