...
当前机器可读入口是 `PILOT-W00`；`FE-W01` 因外部凭据证明和 ptrace 环境
阻断而保留为 `blocked`，并未被描述为完成。
...
文件名使用英文 kebab-case，Wave ID 使用 `FE-WNN`。文件移动不改变
Wave ID。一个问题使用稳定 `FE-ISSUE-NNN`，不能因为跨 Wave 就重新编号。
...
| [`FE-W01`](frontend-stability/fe-w01/plan-r009.md) | `active` | 可追溯 R7、登录、WebSocket transport、inert-provider Gate 与本地 Playwright 回归 | 受限 |
...
| [`PILOT-W00`](pilot-readiness/pilot-w00/plan-r003.md) | `active` | Runner、治理、前端、恢复与三人并发试点 | 受限 |
