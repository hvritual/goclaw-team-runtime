...
| `TC-ISSUE-001` | 中央全局治理 | 现有 TeamControl 文件存储与 RPC | 管理预算、知识源、Skill、Runner release 和 Context Bundle | r002 实现补齐基础能力，但 exact review 复现跨项目裸 ID、Context target identity、secret schema、CRUD/UI Evidence 与 traceability P1 | `S1` | `fixing` | [`TC-EVID-W01-001`](team-runtime/tc-w01/s05-deterministic-verification.md) collecting；[`TC-EVID-W01-002`](team-runtime/tc-w01/s05-independent-review-r002.md) failed | `TC-W01-S06`–`S08` | r003 activation 后冻结 |
