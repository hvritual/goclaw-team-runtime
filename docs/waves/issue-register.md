...
| `RN-ISSUE-001` | Runner 生命周期 | 现有 register/doctor/work/update/key rotation | 兼容性协商、双 execution profile、校验更新、原子升级与回滚 | 本地执行闭环已实现；版本管理仍依赖人工替换，非 Linux 只有控制 CLI，无法显式采用目录限定降级模式 | `S1` | `fixing` | [`RN-W01 r002`](team-runtime/rn-w01/plan-r002.md) 激活；exact Task Freeze 待产生 | `RN-W01-S01`–`S08` | 待冻结 |
