...
| `TC-ISSUE-001` | 中央全局治理 | 现有 TeamControl 文件存储与 RPC | 管理预算、知识源、Skill、Runner release 和 Context Bundle | 成员/Token/项目/策略已实现；`types/state/registries/gateway` 静态合同确认上述集中 Registry、账本与编译合同不存在 | `S1` | `root-caused` | [`TC-W01 r002`](team-runtime/tc-w01/plan-r002.md) 的缺口、数据和 RBAC 合同；产品尚未修改 | `TC-W01` | activation 后冻结 |
