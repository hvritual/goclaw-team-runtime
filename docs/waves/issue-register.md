...
| `FE-ISSUE-007` | WeWork channel test credential hygiene | 版本化 0.7.0 source；有效性未知 | 脱敏检查 `weworkwsbot_test.go` config literals | 测试只含明显 synthetic placeholder；owner 证明旧值已撤销/轮换或从未有效 | 源码直接包含 35/43 字符 credential-shaped Bot/Secret literals；未证明有效或已被使用 | `S0` | `fixing` | [`FE-EVID-W01-009`](frontend-stability/fe-w01/s04-full-go-channels-reproduction.md)；内容不复制，外部有效性未验证 | `FE-W01-S07` hygiene + `FE-W01-S08` owner closure；external owner blocker；FE-W05 revalidation | `FE-W01-TRANSPORT-R1` rev 7；credential owner action pending |
