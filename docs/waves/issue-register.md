...
| `TC-ISSUE-001` | 中央全局治理 | 现有 TeamControl 文件存储与 RPC | 管理预算、知识源、Skill、Runner release 和 Context Bundle | r004 已在规范化前拒绝路径首尾空白，并保留全部跨平台/RBAC 回归 | `S1` | `verified` | [`TC-EVID-W01-018`](team-runtime/tc-w01/s11-r004-independent-review-8.md) 保留失败；[`019`](team-runtime/tc-w01/s11-r004-review-remediation-9.md) deterministic passed；[`020`](team-runtime/tc-w01/s12-r004-final-verification.md) exact 三路 PASS | `TC-W01-S09`–`S12` | `TC-W01-ACCEPTANCE-006` r004 |
| `TC-ISSUE-002` | Team Control 知识权威 | base `d6a166c` 静态源码；未访问真实运行时 | Team Control 唯一决定规则、知识 revision/approval/visibility 和 Context；正文/索引无第二真相源 | `KnowledgeSource`、Memory Catalog 和 Harness proposal 是分离写入/审批模型，Agent 可直接注入 Catalog，ExecutionPack/Evidence 未绑定 Context/citation | `S1` | `unverified`（合同差距，非运行时缺陷复现） | [`TC-EVID-W02-001`](team-runtime/tc-w02/current-state-responsibility-matrix.md) | `TC-W02`；实现候选 `TC-W03`–`TC-W06` | `TC-W02-REPLAN-001` manual docs binding |
| `RN-ISSUE-001` | Runner 生命周期 | 当前源码 base `d6a166c` | 兼容性协商、双 execution profile、校验更新、原子升级与回滚 | 产品实现提交存在，但 r004 deterministic/independent acceptance 尚未登记；用户先改变控制面路线 | `S1` | `deferred` | [`RN-W01 journal`](team-runtime/rn-w01/journal.md) superseded event | `RN-W01` superseded；未来另起 Wave | 原 freeze 只读保留 |
| `INT-ISSUE-001` | Runner 项目上下文 | 现有 Memory Catalog/Harness knowledge/Codex runner | Runner 以 project-scoped MCP 读取批准知识与 Skill，并验证 Context Bundle | 知识接口存在于 Agent/Gateway，但 Runner/Codex 执行包没有统一 MCP/Context 合同 | `S1` | `deferred` | [`TC-EVID-W02-001`](team-runtime/tc-w02/current-state-responsibility-matrix.md) 静态合同盘点 | `INT-W01` superseded；`TC-W03`–`TC-W06` | 未冻结 |
