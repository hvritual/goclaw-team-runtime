...
//
// Collaboration responses require every known field and validate known enum
// and timestamp shapes. Callers reject malformed responses instead of
// replacing them with an empty array, because an empty fallback would make a
// transport/schema regression look like a real empty timeline. Object
// schemas remain `.loose()` so additive legacy/server fields survive without
// weakening the required known contract.
// ---------------------------------------------------------------------------
...
// rest alone.
const TimelineActorSchema = z.enum(["member", "agent", "system"]);
const CommentTimelineEntrySchema = z.object({
  type: z.literal("comment"),
  id: z.string().min(1),
  actor_type: TimelineActorSchema,
  actor_id: z.string().min(1),
  created_at: z.iso.datetime({ offset: true }),
  content: z.string(),
  parent_id: z.string().nullable(),
  updated_at: z.iso.datetime({ offset: true }),
  comment_type: z.enum(["comment", "status_change", "progress_update", "system"]),
...
const ActivityTimelineEntrySchema = z.object({
  ...
  coalesced_count: z.number().int().positive().optional(),
}).loose();
export const TimelineEntrySchema = z.union([CommentTimelineEntrySchema, ActivityTimelineEntrySchema]);
...
export const SubscribedResponseSchema = z.object({ subscribed: z.boolean() }).loose();
