...
export function canEditSkill(_skill: Skill, ctx: PermissionContext): Decision {
  if (ctx.userId === null) {
  ...
  return deny(
    "not_admin_role",
    "Only workspace owners and admins can edit this skill."
  );
}
...
export function canViewPermissionManagement(ctx: PermissionContext): Decision {
  if (isAdminLike(ctx.role)) return ALLOW;
  return deny(
    "not_admin_role",
