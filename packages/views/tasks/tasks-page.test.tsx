...
const mocks = vi.hoisted(() => ({
  query: { data: [] as Task[], isLoading: false, isError: false, error: null as Error | null },
  create: vi.fn(),
  update: vi.fn(),
  archive: vi.fn(),
  restore: vi.fn(),
...
vi.mock("@multica/core/tasks", () => ({
  taskListOptions: () => ({ queryKey: ["tasks"] }),
...
describe("TasksPage", () => {
  beforeEach(() => {
    mocks.query = { data: [], isLoading: false, isError: false, error: null };
    mocks.mutationError = null;
    vi.clearAllMocks();
  });

  it("renders a visible revision conflict state", () => {
    mocks.query = { data: [TASK], isLoading: false, isError: false, error: null };
  ...
  it("renders distinct empty, denied, and generic error states", () => {
    ...
    mocks.query = { data: [], isLoading: false, isError: true, error: new Error("403 permission denied") };
    const denied = renderWithI18n(<TasksPage />);
    expect(screen.getByText("You do not have access to tasks")).toBeInTheDocument();
    denied.unmount();

    mocks.query = { data: [], isLoading: false, isError: true, error: new Error("network unavailable") };
    renderWithI18n(<TasksPage />);
  ...
  it("archives a terminal task using its current revision", () => {
    mocks.query = { data: [{ ...TASK, status: "done", revision: 3, completed_at: "2026-08-20T00:00:00Z" }], isLoading: false, isError: false, error: null };
    renderWithI18n(<TasksPage />);
  ...
  it("restores an archived task using its current revision", () => {
    mocks.query = { data: [{ ...TASK, status: "archived", revision: 4, restore_status: "done", archived_at: "2026-08-21T00:00:00Z" }], isLoading: false, isError: false, error: null };
    renderWithI18n(<TasksPage />);
  ...
  it("reorders the visible list as one revision-checked command", () => {
    mocks.query = { data: [TASK, { ...TASK, id: "task-2", title: "Follow up", position: 20, revision: 5 }], isLoading: false, isError: false, error: null };
    renderWithI18n(<TasksPage />);
