...
func (m *Manager) Save(session *Session) error {
	// Saving is a write operation even though it only reads the projection:
	// concurrent saves must not share a temporary path or publish an older
	...
	// A unique private temporary file prevents concurrent conversations and
	// crash leftovers from aliasing one another.
	file, err := os.CreateTemp(m.baseDir, "."+filepath.Base(filePath)+".tmp-*")
	if err != nil {
		return err
	}
	tmpPath := file.Name()
