...
version metadata match the exact `DDD_SCAFFOLD_VERSION` declared in the
Makefile. `make verify-ddd-tools` repeats that check before generation; an
unpinned ambient binary must never generate repository artifacts.

- Never edit `server/gen/`, files marked `Code generated by dddgen`, or `.dddgen`
  reconciliation state by hand. Application method bodies marked user-owned are
  the exception and must be preserved by reconciliation.
